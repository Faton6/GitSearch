...
class TempFolderManager:
    ...
    def should_cleanup(self) -> bool:
        ...
        return (
            self._stats.total_size_bytes > self.max_size_bytes or
            self._stats.repo_count > self.max_repos
        )
    ...
    def cleanup_lru(self) -> Tuple[int, int]:
        ...
        with self._lock:
            ...
            repos_to_remove = max(
                0,
                self._stats.repo_count - self.cleanup_keep_newest
            )
            ...
            for repo_url, cache_entry in sorted_repos:
                ...
                try:
                    ...
                    if path.exists():
                        ...
                        logger.debug(
                            f"LRU cleanup: removed {repo_url}, "
                            f"freed {cache_entry.size_bytes / (1024**2):.2f}MB"
                        )
                except Exception as e:
    ...
    def cleanup_all(self) -> Tuple[int, int]:
        ...
        with self._lock:
            try:
                ...
                logger.info(
                    f"Full cleanup: removed {repos_removed} repos, "
                    f"freed {bytes_freed / (1024**3):.2f}GB"
                )
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            'usage_percent': (self._stats.total_size_bytes / self.max_size_bytes * 100) if self.max_size_bytes > 0 else 0,
