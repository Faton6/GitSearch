...
def _decode_report_data(encoded_data: str) -> dict:
    """Decode base64 encoded report data.
...
class ReportDataGenerator:
    ...
    def calculate_leak_type_analysis(self, leaks: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for leak in leaks:
            top_leaks.append(
                (leak.get("url", ""), leak.get("leak_type", ""), leak.get("level", 0), leak.get("found_at", ""))
            )
    ...
    def calculate_company_analysis(self, leaks: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for company, stats in company_breakdown.items():
            company_breakdown_list.append((company, stats["total"], stats["resolved"], stats["pending"]))
    ...
    def calculate_monthly_trends(self, leaks: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for month_key, stats in sorted_months:
            ...
            data["monthly_trends"].append(
                {"month": month_name, "incidents": incidents, "resolved": resolved, "efficiency": efficiency}
            )
    ...
    def calculate_technical_metrics(self, leaks: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        risk_levels = {  # pragma: allowlist secret
            "API_KEYS": "high",  # pragma: allowlist secret
            "DATABASE_CREDENTIALS": "critical",  # pragma: allowlist secret
            "PRIVATE_KEYS": "critical",
            "DEBUG_INFO": "medium",
            "OTHER": "medium",
        }

        common_locations = {  # pragma: allowlist secret
            "API_KEYS": ["config/", "src/", ".env"],  # pragma: allowlist secret
        ...
        for category, stats in leak_type_analysis.items():
            avg_confidence = (
                sum((level + 1) * 25 for level in stats["levels"]) / len(stats["levels"]) if stats["levels"] else 50
            )
        ...
        for report in raw_reports:
            ...
            if isinstance(decoded_ai, dict):
                ...
                if "filters" in decoded_ai:
                    ...
                    if (
                        "too large" in filters_str
                        or "слишком большой" in filters_str
                        or "repository is too big" in filters_str
                    ):
