...
class AdaptiveThreadPoolManager:
    ...
    def __init__(
        ...
        self.monitor_thread = threading.Thread(
            target=self._monitor_and_adjust, daemon=True, name=f"AdaptiveThreadMonitor-{task_type}"
        )
        self.monitor_thread.start()

        logger.info(
            "Adaptive thread pool '%s' started with %d workers (min=%d, max=%d)",
            task_type,
    ...
    def _get_quota_info(self) -> Dict[str, any]:
        ...
        try:
            ...
            avg_remaining_pct = (
                sum(((s["limit"] - s["remaining"]) / s["limit"] * 100) if s["limit"] > 0 else 100 for s in status)
                / len(status)
                if status
                else 0
            )
            ...
            logger.debug("Could not get quota info: %s", e)
            return {
                "available": True,
                "total_remaining": 5000,
    ...
    def _adjust_workers(self, new_worker_count: int):
        ...
        with self.lock:
            ...
            logger.info("Adjusted '%s' thread pool: %d -> %d workers", self.task_type, old_count, self.current_workers)
        ...
        while not self.should_stop:
            try:
                ...
                if optimal_workers != self.current_workers:
                    quota_info = self._get_quota_info()
                    logger.debug(
                        "Quota status: %d remaining, %.1f%% used, %d blocked",
                        quota_info["total_remaining"],
                        quota_info["avg_remaining_pct"],
                        quota_info["blocked_tokens"],
                    )
                    self._adjust_workers(optimal_workers)

            except Exception as e:
                logger.error("Error in adaptive thread monitor: %s", e)
    ...
    def shutdown(self, wait: bool = True):
        ...
        """
        logger.info("Shutting down adaptive thread pool '%s'", self.task_type)
        ...
        logger.info(
            "Thread pool '%s' statistics:\n"
            "  Total tasks: %d\n"
            "  Completed: %d\n"
            "  Adjustments made: %d\n"
            "  Final workers: %d",
            self.task_type,
            self.stats.total_tasks_submitted,
            self.stats.total_tasks_completed,
            self.stats.adjustments_made,
            self.stats.current_workers,
        )
...
def get_scan_pool(initial_workers: int = 5) -> AdaptiveThreadPoolManager:
    ...
    if _scan_pool is None:
        _scan_pool = AdaptiveThreadPoolManager(
            initial_workers=initial_workers, task_type="scan", min_workers=1, max_workers=10
        )
    return _scan_pool
