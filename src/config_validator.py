...
def _create_dataclass_config(data: Dict) -> GitSearchConfigData:
    ...
    return GitSearchConfigData(
        ...
        temp_folder=TempFolderConfigData(**data.get('temp_folder', {})) if 'temp_folder' in data else TempFolderConfigData(),
...
def _load_env_overrides(prefix: str) -> Dict[str, Any]:
    ...
    for env_key, (section, field_name) in env_mappings.items():
        ...
        if value:
            ...
            elif field_name in ('timeout', 'max_context_length', 'repo_max_size_kb', 'max_time_to_clone',
                          'worker_pool_size', 'worker_queue_size', 'max_repos_to_keep'):
