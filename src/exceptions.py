...
class CloneError(ScanError):
    ...
    def __init__(
        self, url: str, attempt: int = 1, max_attempts: int = 3, reason: str = "", details: Optional[dict] = None
    ):
...
class APIError(GitSearchError):
    ...
    def __init__(
        self, message: str, status_code: Optional[int] = None, provider: str = "", details: Optional[dict] = None
    ):
...
def scan_error_handler(scanner_name: str, url: str):
    ...
    try:
        ...
        raise ScannerNotInstalledError(scanner_name=scanner_name, details={"error": str(e)}) from e
    except PermissionError as e:
        raise RepositoryAccessDeniedError(url=url, reason="Permission denied", details={"error": str(e)}) from e
...
def api_error_handler(provider: str):
    ...
    try:
        ...
        raise APIError(
            message=f"API request timeout for {provider}", provider=provider, details={"error": str(e)}
        ) from e
    except requests.exceptions.ConnectionError as e:
        raise APIError(message=f"Connection error for {provider}", provider=provider, details={"error": str(e)}) from e
    except Exception as e:
        raise APIError(
