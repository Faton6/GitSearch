...
class GlistScan:
    ...
    def run(cls, filter_: str = '', quantity: int = 15):
        ...
        gist_search = UniversalGitHubSearch(
            search_type=SearchType.GISTS,
            use_graphql=False  # GraphQL doesn't support Gist search
        )
                    ...
                    for gist_url in glist_urls:
                        try:
                            ...
                            if gist_url not in checked_list:
                                ...
                                constants.RESULT_MASS['Glist_scan'][checked_list[gist_url].repo_name] = \
                                    checked_list[gist_url]
                            
                            constants.quantity_obj_before_send += 1
                            
                            # Dump data if needed
                            if (constants.quantity_obj_before_send >= constants.MAX_OBJ_BEFORE_SEND or
                                    constants.dork_search_counter > constants.MAX_SEARCH_BEFORE_DUMP and 
                                    len(constants.RESULT_MASS)):
                                utils.dumping_data()
                                utils.check_temp_folder_size()
