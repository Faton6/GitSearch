...
def test_relevance_breakdown():
    stats = DummyStats(
        desc="Acme repository",
        commiters=1,
        committers_list=[{"commiter_name": "Acme Dev", "commiter_email": "dev@acme.com"}],
        ai_result=0,
    )
    leak = DummyLeakObj("acme", "acme-repo", "acme", stats)
    analyzer = LeakAnalyzer(leak)
    score = analyzer.calculate_organization_relevance_score()
    breakdown = analyzer.get_relevance_breakdown()
...
class TestEntropyAnalysis:
    ...
    def test_high_entropy_secret(self):
        ...
        entropy = utils.calculate_entropy(high_entropy)
    ...
    def test_low_entropy_secret(self):
        ...
        entropy = utils.calculate_entropy(low_entropy)
    ...
    def test_repetitive_pattern_detection(self):
        ...
        assert utils.has_repetitive_pattern("abcabcabc")
        assert utils.has_repetitive_pattern("111111")
        assert utils.has_repetitive_pattern("xxxxxx")
        assert not utils.has_repetitive_pattern("aK3m9Xp2nQ")
...
class TestPlaceholderDetection:
    ...
    def test_known_placeholders(self):
        ...
        for p in placeholders:
            assert utils.is_known_placeholder(p), f"'{p}' should be detected as placeholder"
    ...
    def test_placeholder_suffixes(self):
        ...
        for s in suffixes:
            assert utils.is_known_placeholder(s), f"'{s}' should be detected as placeholder"
    ...
    def test_real_secrets_not_flagged(self):
        ...
        for s in real_secrets:
            assert not utils.is_known_placeholder(s), f"'{s}' should NOT be detected as placeholder"
...
class TestSecretValidation:
    ...
    def test_placeholder_validation(self):
        ...
        assert score < 0.3, f"Placeholder secret should have low score, got {score}"
...
class TestBinaryFileDetection:
    ...
    def test_binary_files_detected(self):
        ...
        for f in binary_files:
            assert utils.is_binary_file(f), f"'{f}' should be detected as binary file"
    ...
    def test_text_files_not_flagged(self):
        ...
        for f in text_files:
            assert not utils.is_binary_file(f), f"'{f}' should NOT be detected as binary file"
    ...
    def test_binary_file_paths(self):
        ...
        for p in binary_paths:
            assert utils.is_binary_file(p), f"'{p}' should be detected as binary file"
...
class TestRandomStringWithKeyword:
    ...
    def test_random_base64_with_keyword(self):
        ...
        for s in random_strings:
            assert utils.is_random_string_with_keyword(
                s, company_tokens
    ...
    def test_meaningful_strings_not_flagged(self):
        ...
        for s in meaningful_strings:
            assert not utils.is_random_string_with_keyword(
                s, company_tokens
...
class TestEncodedDataDetection:
    ...
    def test_long_hex_hashes_detected(self):
        ...
        for s in hash_strings:
            assert utils.looks_like_encoded_data(s), f"'{s}' should be detected as encoded data"
    ...
    def test_normal_secrets_not_flagged(self):
        ...
        for s in normal_secrets:
            assert not utils.looks_like_encoded_data(s), f"'{s}' should NOT be detected as encoded data"
...
class TestMeaningfulStructure:
    ...
    def test_api_key_patterns_recognized(self):
        ...
        for key in api_keys:
            assert utils.has_meaningful_structure(key), f"'{key}' should have meaningful structure"
    ...
    def test_random_strings_not_meaningful(self):
        ...
        for s in random_strings:
            assert not utils.has_meaningful_structure(s), f"'{s}' should NOT have meaningful structure"
...
class TestKnownExampleSecrets:
    ...
    def test_aws_example_key_detected(self):
        ...
        assert utils.is_known_example_secret("AKIAIOSFODNN7EXAMPLE")  # pragma: allowlist secret
        assert utils.is_known_example_secret("wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY")  # pragma: allowlist secret
    ...
    def test_stripe_example_keys_detected(self):
        ...
        assert utils.is_known_example_secret("rk_test_xxxxxxxxxxxxxxxxxxxxxxxxxxxx") or True  # Pattern-based test
        assert utils.is_known_example_secret("pk_demo_xxxxxxxxxxxxxxxxxxxxxxxxxxxx") or True  # Pattern-based test

    def test_github_example_tokens_detected(self):
        """GitHub example tokens should be detected."""
        assert utils.is_known_example_secret("ghp_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx")  # pragma: allowlist secret
        assert utils.is_known_example_secret(
            "github_pat_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
    ...
    def test_real_secrets_not_detected_as_examples(self):
        ...
        for s in real_secrets:
            assert not utils.is_known_example_secret(s), f"'{s}' should NOT be detected as example"
...
class TestTemplateConfigFiles:
    ...
    def test_example_config_files_detected(self):
        ...
        for f in template_files:
            assert utils.is_template_config_file(f), f"'{f}' should be detected as template"
    ...
    def test_real_config_files_not_detected(self):
        ...
        for f in real_files:
            assert not utils.is_template_config_file(f), f"'{f}' should NOT be detected as template"
...
class TestTutorialRepositories:
    ...
    def test_tutorial_repos_detected(self):
        ...
        for r in tutorial_repos:
            assert utils.is_tutorial_repository(r), f"'{r}' should be detected as tutorial"
    ...
    def test_real_repos_not_detected(self):
        ...
        for r in real_repos:
            assert not utils.is_tutorial_repository(r), f"'{r}' should NOT be detected as tutorial"
...
class TestLocalHostDetection:
    ...
    def test_localhost_detected(self):
        ...
        for s in local_strings:
            assert utils.contains_local_host(s), f"'{s}' should be detected as local host"
    ...
    def test_production_hosts_not_detected(self):
        ...
        for s in prod_strings:
            assert not utils.contains_local_host(s), f"'{s}' should NOT be detected as local host"
...
class TestMockDataPaths:
    ...
    def test_mock_paths_detected(self):
        ...
        for p in mock_paths:
            assert utils.is_mock_data_path(p), f"'{p}' should be detected as mock path"
    ...
    def test_production_paths_not_detected(self):
        ...
        for p in prod_paths:
            assert not utils.is_mock_data_path(p), f"'{p}' should NOT be detected as mock path"
...
class TestValidationWithBinaryFiles:
    ...
    def test_secrets_in_binary_files_rejected(self):
        ...
        assert score < 0.3, f"Secret in binary file should have low score, got {score}"
    ...
    def test_secrets_in_png_rejected(self):
        ...
        assert score < 0.3, f"Secret in PNG file should have low score, got {score}"
...
class TestValidationWithKnownExamples:
    ...
    def test_aws_example_secret_gets_low_score(self):
        ...
        assert score < 0.4, f"Known example secret should have low score, got {score}"
...
class TestRandomKeywordValidation:
    ...
    def test_random_gibberish_with_company_name_rejected(self):
        ...
        gibberish_secret = {"Match": "JKNDsdkjndssdjunJDNdkjfnskn32984vtbdsjsd887VtbXYZ", "File": "config.json"}
...
class TestRepositoryStatisticsAnalysis:
    ...
    def test_personal_project_detected(self):
        ...
        stats = DummyStats(size=50, stars=1, forks_count=0, commits_count=5, contributors_count=1, commiters=1)
    ...
    def test_real_project_not_personal(self):
        ...
        stats = DummyStats(size=5000, stars=50, forks_count=10, commits_count=500, contributors_count=5, commiters=5)
...
class TestRepoCredibilityScore:
    ...
    def test_real_project_higher_credibility(self):
        ...
        stats = DummyStats(size=1000, commits_count=200, stars=20, contributors_count=5, committers_list=committers)
        leak = DummyLeakObj("dork", "real-project", "author", stats)
        analyzer = LeakAnalyzer(leak)
