...
def test_dump_from_DB_success_mode_0():
    with patch("src.Connector.APIClient.get_data") as mock_get_data:
        mock_get_data.return_value = [{"id": 1, "url": "url1", "result": 4}, {"id": 2, "url": "url2", "result": 0}]
...
def test_get_company_name_success():
    with patch("src.Connector.APIClient._make_request") as mock_make_request:
        mock_make_request.return_value = {"auth": True, "content": [{"name": "Acme"}]}
...
def test_get_company_name_no_connection():
    with patch("src.Connector.APIClient._make_request") as mock_make_request:
        mock_make_request.return_value = {"auth": True, "content": [0]}  # Empty result indicator
...
def test_dump_account_from_DB_success():
    with patch("src.Connector.APIClient.get_data") as mock_get_data:
        mock_get_data.return_value = [{"account": "account1"}, {"account": "account2"}]
        ...
        "scan": {
            "1": [
                ...
                },
                {
                    ...
                    "content": {
                        "leak_id": 1,
                        "report_name": "repo_url_1",
                        "raw_data": "raw_data_1",
                        "ai_report": "ai_report_1",
                    },
                },
                {
                    "size": 100,
...
def test_dump_to_DB_mode_0_success(
    mock_load_urls, mock_strftime, mock_json_dump, mock_open, mock_logger, mock_dump_to_DB_req
):
...
def test_load_existing_leak_urls():
    with patch("src.Connector.APIClient.get_data") as mock_get_data:
        mock_get_data.return_value = [{"id": 1, "url": "url1"}, {"id": 2, "url": "url2"}]
...
def test_update_existing_leak(mock_get_data, mock_add_data):
    mock_get_data.side_effect = [[{"id": 5, "url": "url1", "result": "4", "updated_at": "2024-01-01"}], []]
