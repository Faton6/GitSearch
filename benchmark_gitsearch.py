...
from src.searcher.scanner import Scanner  # noqa: E402
...
class GitSearchBenchmark:
    ...
    def _benchmark_deep_scan(self, company_metrics: CompanyMetrics, repos: set, company_name: str):
        ...
        for repo_url in list(repos)[: self.config.get("max_repos", 100)]:
            try:
                ...
                logger.info(
                    f"[{scanned_count}/{len(repos)}] Scanned {repo_url}: "
    ...
    def _process_findings(self, leak_obj, company_metrics: CompanyMetrics):
        ...
        if hasattr(leak_obj, "res_check"):
            if leak_obj.res_check in [constants.RESULT_CODE_STILL_ACCESS, constants.RESULT_CODE_TO_SEND]:
    ...
    def _update_resource_metrics(self):
        ...
        try:
            temp_size = (
                sum(f.stat().st_size for f in Path(constants.TEMP_FOLDER).rglob("*") if f.is_file()) / 1024 / 1024
            )
    ...
    def _write_markdown_report(self, f):
        ...
        for company_name, metrics in m.company_results.items():
            f.write(
                f'| {company_name} | {metrics["repos_scanned"]} | '
...
def main():
    ...
    parser.add_argument("--companies", type=str, help="Comma-separated company IDs to test (default: all)")
    parser.add_argument(
        "--max-repos", type=int, default=100, help="Maximum repositories to scan per company (default: 100)"
    )
    parser.add_argument("--max-dorks", type=int, default=50, help="Maximum dorks to test per company (default: 50)")
    parser.add_argument("--skip-ai", action="store_true", help="Skip AI analysis to speed up testing")
    parser.add_argument(
        "--output",
        type=str,
    ...
    parser.add_argument("--db-host", type=str, help="Database host (default: from config.json)")
    parser.add_argument("--db-token", type=str, help="Database token (default: from config.json)")
